...
require('electron-reload')(path.join(__dirname, 'views'))//Really need?
                           
app.on('ready', () => {
    win = new BrowserWindow()
    win.loadURL('file:///home/pdro/Paralelos/PNPNG/views/teacher.pug') // use path?
    fs.readFile('questions.json', (err, results) => {
        if (err) throw err
        exports.questions = JSON.parse(results)
    })
    console.log(exports.questions)
})
